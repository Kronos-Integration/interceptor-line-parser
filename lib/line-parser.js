			...
			for (let i = start; i < buf.length; i++) {
				...
				if (!this._quoting) {
					...
					if (buf[i] === this.newline) {
						if (buf[i - 2] === CR) {
							this._pushLine(buf.slice(this._prevEnd, i - 2)); // -1 we do not need the CR and new line
						} else {
							this._pushLine(buf.slice(this._prevEnd, i - 1)); // -1 we do not need the new line
						}
